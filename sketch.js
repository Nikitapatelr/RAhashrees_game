...
function draw() {
  ...
  if(gameState==="Stage1"){
    ...
    text("Mission LUNA begins !",500,20);
    textSize(21);
    text("The Rocket's about to leave !! Press the Right Arrow to take off !!",340,70);

    if(World.seconds>9){
      fill("red");
      text("Your time has started",50,100);
    }

    fill(255,200,0);
    textSize(22);
    text("You only have 3 MINUTES to complete this Mission",370,47);

    if(keyDown(RIGHT_ARROW)){
      astronaut.x += 10;
      text("You're about to face a massive Asteroid Attack ! Watch Out !",360,93);
    }
  ...
  if(gameState==="Stage3"){
    ...
    fill(175,130,160);
    textSize(22);
    text("MISSION:Destroy the Neighbouring Country's Spying Satellites.Move your CURSOR and press SPACE",astrofly.x-450,22);
    text("TARGET:1650.Limited Bullets",astrofly.x-150,44);
    ...
    if(World.seconds===60){
      ...
      stroke(0);
      textSize(22);
      text("2 MINUTES LEFT",astrofly.x-70,65);
    }
    if(World.seconds===120){
      fill(0,255,255);
      stroke(0);
      textSize(22);
      text("1 MINUTE LEFT",astrofly.x-70,65);
    }
    if(World.seconds===150){
      fill(0,255,255);
      stroke(0);
      textSize(22);
      text("30 SECONDS LEFT",astrofly.x-70,65);
    }
    ...
    // STAGE OVER
    if(World.seconds===190 || score<700 || bullets<0){
      gameState = "over";
      over.play();
      back.stop();
    }
  ...
  if(gameState==="Stage4"){
    ...
    text("Use ARROW keys to EXPLORE (UP,DOWN,LEFT,RIGHT)",160,270);
    text("Start with the RIGHT ARROW KEY",160,290);

    fill(0,255,255);
    textSize(22);
    text("SCORE: "+score,160,50);
    text("SAMPLES COLLECTED: "+icescore,160,70);
    satgroup.destroyEach();
    ...
    if(keyDown(LEFT_ARROW)){
      text("<-You're going left",astronaut.x+50,astronaut.y);
      astronaut.x = astronaut.x-10;
    }
    if(keyDown(RIGHT_ARROW)){
      text("You're going right->",astronaut.x-230,astronaut.y);
      astronaut.x = astronaut.x+10;
    } 
    if(keyDown(UP_ARROW)){
